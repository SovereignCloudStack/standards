...
`-GXn[:m][h]` indicates a Pass-Through GPU from vendor X of gen n with m compute units / SMs exosed.
`-gXn[:m][h]` indicates a vGPU from vendor X of gen n with m compute units / SMs assigned.
